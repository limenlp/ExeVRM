...
class ThinkingBudgetLogitsProcessor:
    r"""Force the model to emit the think-end token after a thinking budget is exceeded.

    At each generation step, scans the current sequence for the last think-start token.
    If the number of tokens generated after it exceeds ``thinking_budget`` and no think-end
    token has been emitted yet, all logits are masked except for the think-end token.
    """

    def __init__(self, think_start_token_id: int, think_end_token_id: int, thinking_budget: int) -> None:
    ...
    def __call__(self, input_ids: "torch.LongTensor", scores: "torch.FloatTensor") -> "torch.FloatTensor":
        for b in range(input_ids.shape[0]):
            ...
            think_start_mask = seq == self.think_start_token_id
            if not think_start_mask.any():
                continue

            last_think_pos = think_start_mask.nonzero(as_tuple=True)[0][-1].item()

            # Check if </think> already appears after the last <think>
            tail = seq[last_think_pos + 1 :]
            if (tail == self.think_end_token_id).any():
                continue  # thinking already closed

            # Count thinking tokens (everything after <think>)
            thinking_tokens = len(tail)
            if thinking_tokens >= self.thinking_budget:
                # Force </think> token
                scores[b, :] = float("-inf")
                scores[b, self.think_end_token_id] = 0.0
