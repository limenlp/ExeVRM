...
def get_dataset(
    ...
    with training_args.main_process_first(desc="pre-process dataset", local=(not data_args.data_shared_file_system)):
        # After barrier, non-main processes check if rank 0 has already saved tokenized data
        if (
            data_args.tokenized_path is not None
            and training_args.local_process_index != 0
            and has_tokenized_data(data_args.tokenized_path)
        ):
            logger.info("Loading tokenized dataset saved by main process from %s.", data_args.tokenized_path)
            dataset_dict = load_from_disk(data_args.tokenized_path)
        else:
