...
from ..model.model_utils.stp import get_rope_index_with_stp
...
class MultiModalDataCollatorForSeq2Seq(DataCollatorForSeq2Seq):
    ...
    def __call__(self, features: list[dict[str, Any]]) -> dict[str, "torch.Tensor"]:
        ...
        # Collect per-sample STP/TTP settings for forward_removal mode
        batch_use_stp, batch_use_ttp = [], []
        for feature in features:
            ...
            # Pop STP/TTP settings (None means use global settings)
            batch_use_stp.append(feature.pop("_use_stp", None))
            batch_use_ttp.append(feature.pop("_use_ttp", None))
        ...
        if self.get_rope_func is not None:
            # Check if STP position-preserving mode is active
            use_stp_positions = (
                "stp_token_positions" in mm_inputs and "stp_num_selected" in mm_inputs
            )

            if use_stp_positions and getattr(self.model.config, "model_type", None) in [
                "qwen2_vl",
                "qwen2_5_vl",
                "qwen3_vl",
                "qwen3_vl_moe",
            ]:
                # Use STP-aware rope index computation
                image_processor = getattr(self.processor, "image_processor", None)
                spatial_merge_size = getattr(image_processor, "merge_size", 2) if image_processor else 2
                ...
                # Try to get vision_start_token_id from tokenizer
                tokenizer = getattr(self.processor, "tokenizer", None)
                if tokenizer is not None:
                    vision_start_token_id = tokenizer.convert_tokens_to_ids("<|vision_start|>")

                if image_token_id is not None and vision_start_token_id is not None:
                    features["position_ids"], features["rope_deltas"] = get_rope_index_with_stp(
                        input_ids=features["input_ids"],
                        attention_mask=(features["attention_mask"] >= 1).float(),
                        image_token_id=image_token_id,
                        video_token_id=video_token_id if video_token_id is not None else -1,
                        vision_start_token_id=vision_start_token_id,
                        spatial_merge_size=spatial_merge_size,
                        image_grid_thw=mm_inputs.get("image_grid_thw"),
                        video_grid_thw=mm_inputs.get("video_grid_thw"),
                        stp_token_positions=mm_inputs.get("stp_token_positions"),
                        stp_num_selected=mm_inputs.get("stp_num_selected"),
                        stp_video_token_positions=mm_inputs.get("stp_video_token_positions"),
                        stp_video_num_selected=mm_inputs.get("stp_video_num_selected"),
                    )
                else:
                    # Fallback to default rope function
                    rope_index_kwargs = {
                        "input_ids": features["input_ids"],
                        "image_grid_thw": mm_inputs.get("image_grid_thw"),
                        "video_grid_thw": mm_inputs.get("video_grid_thw"),
                ...
                # Standard rope index computation
                rope_index_kwargs = {
                    "input_ids": features["input_ids"],
                    "image_grid_thw": mm_inputs.get("image_grid_thw"),
                    "video_grid_thw": mm_inputs.get("video_grid_thw"),
        ...
        # Add per-video STP/TTP settings for forward_removal mode
        # Expand per-sample settings to per-video settings based on batch_vidlens
        # This allows each video to have its own STP/TTP setting
        per_video_use_stp = []
        per_video_use_ttp = []
        for i, (use_stp, use_ttp, vidlen) in enumerate(zip(batch_use_stp, batch_use_ttp, batch_vidlens)):
            # Expand the sample's setting to all its videos
            per_video_use_stp.extend([use_stp] * vidlen)
            per_video_use_ttp.extend([use_ttp] * vidlen)
        # Store as lists (not tensors) since they contain None values
        features["_per_video_use_stp"] = per_video_use_stp
        features["_per_video_use_ttp"] = per_video_use_ttp
