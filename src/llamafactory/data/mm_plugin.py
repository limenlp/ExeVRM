...
from ..model.model_utils.stp import apply_stp_token_reduction
...
class MMPluginMixin:
    ...
    def _get_video_sample_indices(
        self, total_frames: int, duration_seconds: float, video_fps: float, video_maxlen: int, **kwargs
    ) -> list[int]:
        r"""Compute video sample indices according to fps."""
        if total_frames == 0:  # infinite video
            return np.linspace(0, video_maxlen - 1, video_maxlen).astype(np.int32)

        sample_frames = max(1, math.floor(duration_seconds * video_fps))
    ...
    def _regularize_videos(self, videos: list["VideoInput"], **kwargs) -> "RegularizedVideoOutput":
        ...
        for video in videos:
            ...
            if _check_video_is_nested_images(video):
                ...
                # Use torchcodec if available, otherwise fall back to av
                if is_torchcodec_available():
                    try:
                        decoder = VideoDecoder(video)
                    except Exception as exc:
                        raise ValueError(f"Failed to open video input {video!r}: {exc}") from exc
                    ...
                    if total_frames is None or total_frames == 0:
                        raise ValueError(f"Video input {video!r} does not contain valid frames.")

                    sample_indices = self._get_video_sample_indices(
                        total_frames, duration_seconds, **kwargs
                    )

                    # Decode frames at sample indices
                    for frame_idx in sample_indices:
                        frame_tensor = decoder[int(frame_idx)]  # Returns tensor of shape [C, H, W]
                        # Convert tensor to PIL Image
                        # torchcodec returns uint8 tensor in [C, H, W] format
                        frame_np = frame_tensor.permute(1, 2, 0).cpu().numpy()  # Convert to [H, W, C]
                    ...
                    try:
                        container_ctx = av.open(video, "r")
                    except av.AVError as exc:  # pragma: no cover - pyav specific error path
                        raise ValueError(f"Failed to open video input {video!r}: {exc}") from exc

                    with container_ctx as container:
                        video_stream = next((stream for stream in container.streams if stream.type == "video"), None)
                        if video_stream is None:
                            raise ValueError(f"Video input {video!r} does not contain a video stream.")

                        total_frames = video_stream.frames
                        duration_seconds = float(video_stream.duration * video_stream.time_base) if video_stream.duration else 0.0
                        sample_indices = self._get_video_sample_indices(
                            total_frames, duration_seconds, **kwargs
                        )
                        container.seek(0)
                    ...
                    raise RuntimeError("Neither torchcodec nor PyAV is available for video decoding.")
...
class Qwen2VLPlugin(BasePlugin):
        ...
        for video in videos:
            ...
            if _check_video_is_nested_images(video):
                ...
                if is_torchcodec_available():
                    ...
                    fps_value = len(sample_indices) / duration_seconds if duration_seconds > 0 else kwargs.get("video_fps", 2.0)
                    fps_per_video.append(fps_value)
                    durations.append(duration_seconds)
                elif is_pyav_available():
                    try:
                        container_ctx = av.open(video, "r")
                    ...
                    with container_ctx as container:
                        ...
                        if video_stream.duration is None:
                            fps_value = kwargs.get("video_fps", 2.0)
                            fps_per_video.append(fps_value)
                            durations.append(len(frames) / fps_value if fps_value else 0.0)
                        else:
                            fps_value = len(sample_indices) / float(video_stream.duration * video_stream.time_base)
                            fps_per_video.append(fps_value)
                            durations.append(float(video_stream.duration * video_stream.time_base))
                else:
                    raise RuntimeError("Neither torchcodec nor PyAV is available for video decoding.")
    ...
    def _get_mm_inputs(
        ...
        if len(images) != 0:
            ...
            # Apply STP token reduction if enabled
            # Note: forward_removal and masking modes handle token reduction in the model forward pass,
            # so we skip preprocessing here for those modes
            use_stp = getattr(processor, "use_stp", False)
            stp_threshold = getattr(processor, "stp_threshold", 0.0)
            stp_mode = getattr(processor, "stp_mode", "preprocess")
            stp_large_comp_threshold: int = getattr(processor, "stp_large_comp_threshold", 0)
            stp_skip_ratio: float = getattr(processor, "stp_skip_ratio", 0.5)
            stp_patch_level: bool = getattr(processor, "stp_patch_level", False)
            stp_patch_to_token_strategy: str = getattr(processor, "stp_patch_to_token_strategy", "any")

            # Only apply preprocessing for "preprocess" mode
            # Note: stp_large_comp_threshold > 0 with preprocess mode is NOT supported
            # because it requires model-level position handling that is not implemented.
            # Use forward_removal mode instead for large_comp_threshold > 0.
            if use_stp and stp_threshold > 0.0 and stp_mode == "preprocess":
                if "pixel_values" in mm_inputs and "image_grid_thw" in mm_inputs:
                    merge_size: int = getattr(image_processor, "merge_size", 2)

                    if stp_large_comp_threshold > 0:
                        # WARNING: preprocess mode with large_comp_threshold > 0 is not fully supported
                        # because it requires model-level position handling.
                        # Fall back to apply_stp_token_reduction which maintains grid structure.
                        import warnings

                        warnings.warn(
                            "STP preprocess mode with stp_large_comp_threshold > 0 is not fully supported. "
                            "Falling back to token reduction mode (pooling). "
                            "For true token removal with large_comp_threshold, use stp_mode='forward_removal'.",
                            UserWarning,
                            stacklevel=2,
                        )
                        # Use token reduction mode instead
                        reduced_pixels, reduced_grid, num_reduced = apply_stp_token_reduction(
                            pixel_values=mm_inputs["pixel_values"],
                            image_grid_thw=mm_inputs["image_grid_thw"],
                            threshold=stp_threshold,
                            skip_ratio=stp_skip_ratio,
                            merge_size=merge_size,
                        )
                        if num_reduced > 0:
                            mm_inputs["pixel_values"] = reduced_pixels
                            mm_inputs["image_grid_thw"] = reduced_grid
                    else:
                        # Use token reduction mode (maintains grid structure via pooling)
                        reduced_pixels, reduced_grid, num_reduced = apply_stp_token_reduction(
                            pixel_values=mm_inputs["pixel_values"],
                            image_grid_thw=mm_inputs["image_grid_thw"],
                            threshold=stp_threshold,
        ...
        if len(videos) != 0:
            ...
            # Apply STP token reduction for videos if enabled
            # Note: forward_removal and masking modes handle token reduction in the model forward pass
            use_stp = getattr(processor, "use_stp", False)
            stp_threshold = getattr(processor, "stp_threshold", 0.0)
            stp_mode = getattr(processor, "stp_mode", "preprocess")
            ...
            # Use forward_removal mode instead for large_comp_threshold > 0.
            if use_stp and stp_threshold > 0.0 and stp_mode == "preprocess":
                pixel_key = "pixel_values_videos" if "pixel_values_videos" in mm_inputs else "pixel_values"
                if pixel_key in mm_inputs and "video_grid_thw" in mm_inputs:
                    merge_size: int = getattr(image_processor, "merge_size", 2)
                    ...
                    if stp_large_comp_threshold > 0:
                        ...
                        # Use token reduction mode instead
                        reduced_pixels, reduced_grid, num_reduced = apply_stp_token_reduction(
                            pixel_values=mm_inputs[pixel_key],
                            image_grid_thw=mm_inputs["video_grid_thw"],
                            threshold=stp_threshold,
                            skip_ratio=stp_skip_ratio,
                            merge_size=merge_size,
                        )
                        if num_reduced > 0:
                            mm_inputs[pixel_key] = reduced_pixels
                            mm_inputs["video_grid_thw"] = reduced_grid
                    else:
                        # Use token reduction mode (maintains grid structure via pooling)
                        reduced_pixels, reduced_grid, num_reduced = apply_stp_token_reduction(
                            pixel_values=mm_inputs[pixel_key],
...
class Qwen3VLPlugin(Qwen2VLPlugin):
    ...
    def _get_mm_inputs(
        ...
        # Precompute TTP/STP keep mask here in the data-loader worker (CPU).
        # Both masks depend only on pixel_values and grid_thw, which are finalised above.
        # Moving computation out of model forward() eliminates GPU CUDA-sync overhead.
        #
        # grid_thw consistency: this mask is computed with the same video_grid_thw that will
        # be passed to patched_forward, so token counts are guaranteed to match.
        _use_ttp = getattr(processor, "use_ttp", False)
        _use_stp_fwd = (
            getattr(processor, "use_stp", False)
            and getattr(processor, "stp_mode", "preprocess") == "forward_removal"
        )
        _pv_key = "pixel_values_videos" if "pixel_values_videos" in mm_inputs else None
        if _pv_key is not None and "video_grid_thw" in mm_inputs and (_use_ttp or _use_stp_fwd):
            _pv = mm_inputs[_pv_key]
            _grid = mm_inputs["video_grid_thw"]
            _img_proc = getattr(processor, "image_processor", None)
            _vid_proc = getattr(processor, "video_processor", _img_proc)
            _merge_sz = getattr(_vid_proc, "merge_size", 2)
            _patch_sz = getattr(_img_proc, "patch_size", 14) if _img_proc else 14
            _temporal_patch_sz = getattr(_img_proc, "temporal_patch_size", 2) if _img_proc else 2

            _ttp_mask = None
            _stp_mask = None

            if _use_ttp:
                from ..model.model_utils.ttp import compute_run_length_keep_mask as _compute_ttp
                _ttp_mask = _compute_ttp(
                    _pv,
                    _grid,
                    threshold=getattr(processor, "ttp_threshold", 0.9),
                    min_run_length=getattr(processor, "ttp_min_run_length", 2),
                    similarity_metric=getattr(processor, "ttp_similarity_metric", "cosine"),
                    patch_size=_patch_sz,
                    temporal_patch_size=_temporal_patch_sz,
                    merge_size=_merge_sz,
                    use_raw_frames_in_ttp=getattr(processor, "use_raw_frames_in_ttp", False),
                    comparison_mode=getattr(processor, "ttp_comparison_mode", "reference"),
                )

            if _use_stp_fwd:
                _stp_thr = getattr(processor, "stp_threshold", 0.0)
                if _stp_thr > 0:
                    from ..model.model_utils.stp import compute_token_keep_mask_from_pixels as _compute_stp
                    _stp_mask = _compute_stp(
                        _pv,
                        _grid,
                        threshold=_stp_thr,
                        skip_ratio=getattr(processor, "stp_skip_ratio", 0.5),
                        large_comp_threshold=getattr(processor, "stp_large_comp_threshold", 0),
                        patch_size=_patch_sz,
                        temporal_patch_size=_temporal_patch_sz,
                        merge_size=_merge_sz,
                        patch_level=getattr(processor, "stp_patch_level", False),
                        patch_to_token_strategy=getattr(processor, "stp_patch_to_token_strategy", "any"),
                        temporal_aggregation=getattr(processor, "stp_temporal_aggregation", "first"),
                        use_raw_frames_in_stp=getattr(processor, "use_raw_frames_in_stp", False),
                    )

            if _ttp_mask is not None and _stp_mask is not None:
                from ..model.model_utils.ttp import combine_stp_and_ttp_masks as _combine_masks
                mm_inputs["video_ttp_keep_mask"] = _combine_masks(_stp_mask, _ttp_mask)
            elif _ttp_mask is not None:
                mm_inputs["video_ttp_keep_mask"] = _ttp_mask
            elif _stp_mask is not None:
                mm_inputs["video_ttp_keep_mask"] = _stp_mask
...
@dataclass
class Molmo2Plugin(BasePlugin):
    r"""Plugin for Molmo2 model that handles video input with proper metadata.

    Molmo2's video processor requires video_metadata with FPS information.
    This plugin bypasses LlamaFactory's video regularization and lets
    Molmo2's processor handle video decoding directly from file paths.

    Molmo2 uses special tokens:
    - <|image|> as image placeholder (replaced with <im_start>...<im_end>)
    - <|video|> as video placeholder (replaced with frame tokens)
    """
    # Molmo2 special tokens
    im_start_token: str = "<im_start>"
    im_end_token: str = "<im_end>"
    im_patch_token: str = "<im_patch>"
    im_col_token: str = "<im_col>"
    ...
    def _get_mm_inputs(
        ...
        r"""Process visual inputs for Molmo2.

        For videos, we pass the raw video inputs (file paths) directly to Molmo2's
        video processor, which will handle decoding and extract FPS metadata itself.
        """
        mm_inputs = {}
        ...
        if len(images) != 0:
            ...
            image_processor_kwargs = {}
            if imglens is not None and hasattr(processor, "process_image_tokens"):
                image_processor_kwargs["imglens"] = imglens

            mm_inputs.update(image_processor(images, return_tensors="pt", **image_processor_kwargs))

        # Process videos - pass raw inputs to Molmo2's processor (don't regularize)
        if len(videos) != 0:
            video_processor = getattr(
                processor, "video_processor", getattr(processor, "image_processor", None)
            )
            # Molmo2's video processor can handle file paths directly and will
            # extract FPS metadata during decoding. We don't call _regularize_videos
            # because that would convert to numpy arrays and lose metadata.
            # Don't request metadata here - it's only needed in process_messages
            mm_inputs.update(video_processor(videos=videos, return_tensors="pt"))
    ...
    def _get_video_tokens(
        ...
        r"""Generate video token string for Molmo2.

        Based on Molmo2Processor.get_video_string() method.
        """
        ...
        for frame_idx, frame_time in enumerate(timestamps):
            ...
            # Build frame tokens: <im_start> + patches + <im_end>
            per_row = self.im_patch_token * w
            if use_col_tokens:
                per_row += self.im_col_token
            patches = per_row * h
            video_string += f"{self.im_start_token}{patches}{self.im_end_token}"
    ...
    def process_messages(
        ...
        r"""Process messages for Molmo2, replacing placeholders with proper tokens.

        Note: We request numpy arrays instead of PyTorch tensors to avoid allocating
        large GPU memory here. The actual GPU allocation happens in _get_mm_inputs.
        """
        self._validate_input(processor, images, videos, audios)
        self._validate_messages(messages, images, videos, audios)
        messages = deepcopy(messages)

        # Get video grid info if we have videos
        # Use return_tensors="np" to avoid GPU memory allocation
        video_grids = None
        video_metadata = None
        if len(videos) != 0 and self.expand_mm_tokens:
            video_processor = getattr(
                processor, "video_processor", getattr(processor, "image_processor", None)
            )
            # Request numpy arrays to avoid GPU allocation, only need grid/metadata info
            video_outputs = video_processor(videos=videos, return_tensors="np", return_metadata=True)
            video_grids = video_outputs.get("video_grids", None)
            video_metadata = video_outputs.get("video_metadata", None)
            # Explicitly delete pixel data to free memory
            if "pixel_values_videos" in video_outputs:
                del video_outputs["pixel_values_videos"]

        # Get image grid info if we have images
        # Use return_tensors="np" to avoid GPU memory allocation
        image_grids = None
        if len(images) != 0 and self.expand_mm_tokens:
            ...
            # Request numpy arrays to avoid GPU allocation
            image_outputs = image_processor(regularized, return_tensors="np")
            image_grids = image_outputs.get("image_grids", None)
            # Explicitly delete pixel data to free memory
            if "pixel_values" in image_outputs:
                del image_outputs["pixel_values"]

        num_image_tokens, num_video_tokens = 0, 0
        use_col_tokens = getattr(processor, "video_use_col_tokens", False)
        ...
        for message in messages:
            ...
            while IMAGE_PLACEHOLDER in content:
                if image_grids is not None and num_image_tokens < len(image_grids):
                    grid = image_grids[num_image_tokens]
                    # grid: [resized_h, resized_w, height, width]
                    if hasattr(grid, 'numpy'):
                        grid = grid.numpy()
                    _, _, h, w = grid
                    per_row = self.im_patch_token * int(w)
                    patches = per_row * int(h)
                    replacement = f"{self.im_start_token}{patches}{self.im_end_token}"
                else:
                    replacement = self.image_token  # fallback
                content = content.replace(IMAGE_PLACEHOLDER, replacement, 1)
                num_image_tokens += 1

            # Replace video placeholders
            while VIDEO_PLACEHOLDER in content:
                if video_grids is not None and video_metadata is not None and num_video_tokens < len(video_grids):
                    grid = video_grids[num_video_tokens]
                    ...
                    if timestamps is None:
                        ...
                        fps = getattr(metadata, 'fps', 1.0) or 1.0
                        timestamps = [i / fps for i in range(num_frames)]
                    replacement = self._get_video_tokens(grid, timestamps, use_col_tokens)
                else:
                    replacement = self.video_token  # fallback
