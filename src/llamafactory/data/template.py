...
@dataclass
class Qwen3VLReasoningTemplate(ReasoningTemplate):
    r"""Custom chat template for Qwen3-VL that matches the official Jinja template."""

    @override
    def _get_jinja_template(self, tokenizer: "PreTrainedTokenizer") -> str:
        vision_start = getattr(self.mm_plugin, "vision_bos_token", "<|vision_start|>")
        vision_end = getattr(self.mm_plugin, "vision_eos_token", "<|vision_end|>")
        image_token = getattr(self.mm_plugin, "image_token", "<|image_pad|>") or "<|image_pad|>"
        video_token = getattr(self.mm_plugin, "video_token", "<|video_pad|>") or "<|video_pad|>"
        ...
        template = (
            ...
            "                    __VISION_START____IMAGE_TOKEN____VISION_END__\n"
            "                {%- elif content.type == 'video' or 'video' in content %}\n"
            "                    {%- set video_count.value = video_count.value + 1 %}\n"
            "                    {%- if add_vision_id %}Video {{ video_count.value }}: {% endif -%}\n"
            "                    __VISION_START____VIDEO_TOKEN____VISION_END__\n"
        ...
        return (
            template.replace("__VISION_START__", vision_start)
            .replace("__VISION_END__", vision_end)
            .replace("__IMAGE_TOKEN__", image_token)
            .replace("__VIDEO_TOKEN__", video_token)
        )
...
# Molmo2 uses a simple user/assistant format without special tokens for roles
# Based on allenai/Molmo2-8B chat template
# Supports image (<|image|>) and video (<|video|>) inputs
# Uses molmo2 plugin which handles video input with proper FPS metadata
# The plugin expands placeholders to proper token sequences with <im_start>...<im_end>
register_template(
    name="molmo2",
    format_user=StringFormatter(slots=["user\n{{content}}\n"]),
    format_assistant=StringFormatter(slots=["assistant\n{{content}}"]),
    mm_plugin=get_mm_plugin(name="molmo2", image_token="<|image|>", video_token="<|video|>"),
)
...
register_template(
    ...
    replace_jinja_template=True,
    template_class=Qwen3VLReasoningTemplate,
)
