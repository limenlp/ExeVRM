...
class FinetuningArguments(
    ...
    compute_video_reward_cumulative_accuracy: bool = field(
        default=False,
        metadata={"help": "Whether or not to compute the cumulative video reward accuracy at each training step."},
    )
    eval_save_predictions: bool = field(
        default=False,
        metadata={"help": "Whether to save eval predictions, ground truths, and compute F1 score to a JSON file."},
    )
    eval_predictions_output_dir: str | None = field(
        default=None,
        metadata={"help": "Directory to save eval predictions JSON file. If not set, uses training output_dir."},
    )
    ...
    use_vllm_eval: bool = field(
        ...
        metadata={
            "help": (
                "Whether to use vLLM for generation during evaluation. "
                "Offloads model to CPU, runs vLLM subprocess, then restores model."
            )
        },
    )
    vllm_eval_max_model_len: int = field(
        default=16384,
        metadata={"help": "Maximum model length (context window) for vLLM evaluation engine."},
    )
    vllm_eval_gpu_util: float = field(
        default=0.45,
        metadata={"help": "Fraction of GPU memory for vLLM eval. Keep low since optimizer states remain on GPU."},
    )
    vllm_eval_tp_size: int = field(
        default=0,
        metadata={"help": "Tensor parallel size for vLLM evaluation engine. 0 = use all available GPUs."},
    )
    vllm_eval_enforce_eager: bool = field(
        default=True,
        metadata={"help": "Disable CUDA graphs in vLLM eval to save GPU memory."},
    )
    vllm_eval_batch_size: int = field(
        default=1024,
        metadata={"help": "Batch size for vLLM evaluation inference."},
    )
    vllm_eval_sleep_mode: bool = field(
        default=True,
        metadata={
            "help": (
                "Whether to use sleep/wake mode for persistent vLLM engine during evaluation. "
                "When True, the engine is kept alive between eval steps and put to sleep "
                "(GPU memory freed) during training. Weights are synced via /dev/shm. "
                "When False, falls back to legacy create/destroy approach."
            )
        },
    )
