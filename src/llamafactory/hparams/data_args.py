...
class DataArguments:
    ...
    mix_strategy: Literal["concat", "interleave_under", "interleave_over", "paired_interleave"] = field(
        default="concat",
        metadata={"help": "Strategy to use in dataset mixing (concat/interleave) (undersampling/oversampling) (paired_interleave for strict A,B,A,B ordering)."},
    )
    interleave_probs: str | None = field(
        default=None,
        metadata={"help": "Probabilities to sample data from datasets. Use commas to separate multiple datasets."},
    )
    paired_interleave_group_size: int | None = field(
        default=None,
        metadata={
            "help": (
                "Number of datasets in paired_interleave mode. "
                "When set, enables paired shuffle that shuffles groups while maintaining order within each group. "
                "For 2 datasets, each group contains one sample from each dataset (A[i], B[i])."
            )
        },
    )
    ...
    def __post_init__(self):
        ...
        if self.paired_interleave_group_size is not None:
            if self.mix_strategy != "paired_interleave":
                raise ValueError("`paired_interleave_group_size` is only valid for `mix_strategy=paired_interleave`.")
            if self.dataset is not None and len(self.dataset) != self.paired_interleave_group_size:
                raise ValueError(
                    f"`paired_interleave_group_size` ({self.paired_interleave_group_size}) "
                    f"must match the number of datasets ({len(self.dataset)})."
                )
        elif self.mix_strategy == "paired_interleave" and self.dataset is not None:
            # Auto-set group_size based on number of datasets
            self.paired_interleave_group_size = len(self.dataset)
