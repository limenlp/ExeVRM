...
class ProcessorArguments:
    ...
    use_stp: bool = field(
        default=False,
        metadata={"help": "Whether to use STP-style UI-guided token selection to reduce visual tokens."},
    )
    stp_threshold: float = field(
        default=3.0,
        metadata={"help": "Patch-wise difference threshold for STP. Larger values result in more token reduction."},
    )
    stp_skip_ratio: float = field(
        default=0.0,
        metadata={"help": "Ratio of patches to skip within each UI component (0.0 to 1.0)."},
    )
    stp_large_comp_threshold: int = field(
        default=10,
        metadata={"help": "Components larger than this are fully skipped (0 = disabled). Enables aggressive reduction."},
    )
    stp_mode: str = field(
        default="forward_removal",
        metadata={
            "help": "STP token reduction mode: "
            "'preprocess' - apply 2x2 pooling in data preprocessing (changes grid_thw); "
            "'masking' - zero out skipped embeddings (preserves grid structure); "
            "'forward_removal' - remove tokens after position IDs computed (true reduction, correct positions)"
        },
    )
    stp_patch_level: bool = field(
        default=True,
        metadata={
            "help": "If True, perform STP analysis at individual patch level (e.g., 48x48) "
            "instead of merged token level (e.g., 24x24). This preserves more fine-grained "
            "information but may keep more tokens."
        },
    )
    stp_patch_to_token_strategy: str = field(
        default="any",
        metadata={
            "help": "Strategy to map patch-level decisions to token-level when stp_patch_level=True. "
            "'any' - keep token if ANY of its patches are kept (conservative); "
            "'all' - keep token only if ALL patches are kept (aggressive); "
            "'majority' - keep token if more than half of patches are kept."
        },
    )
    stp_temporal_aggregation: str = field(
        default="first",
        metadata={
            "help": "How to handle temporal patches when temporal_patch_size > 1 (e.g., video). "
    ...
    use_raw_frames_in_stp: bool = field(
        default=True,
        metadata={
            "help": "If True and the vision encoder uses temporal patches (temporal_patch_size>1), "
            "compute STP keep masks on each raw temporal frame (within each temporal patch group) "
            "and OR them. This can preserve UI elements that appear in any frame."
        },
    )
    ...
    use_ttp: bool = field(
        default=False,
        metadata={
            "help": "Whether to use Temporal Token Pruning to remove temporally repeated visual tokens. "
            "Inspired by 'Don't Look Twice: Faster Video Transformers with Temporal Token Pruning' (NeurIPS 2024)."
        },
    )
    ttp_threshold: float = field(
        default=0.9999,
        metadata={
            "help": "Similarity threshold for Temporal Token Pruning. Tokens with cosine similarity above this "
            "threshold across consecutive temporal frames are considered duplicates. Range: 0.0 to 1.0."
        },
    )
    ttp_min_run_length: int = field(
        default=2,
        metadata={
            "help": "Minimum run length to trigger token removal. A run of N identical tokens will be reduced to 1. "
            "Set to 2 to remove any consecutive duplicates, higher values are more conservative."
        },
    )
    ttp_similarity_metric: str = field(
        default="cosine",
        metadata={
            "help": "Similarity metric for comparing tokens across temporal frames. "
            "'cosine' - cosine similarity (recommended); "
            "'l2' - L2 distance normalized by patch dimension; "
            "'l1' - L1 distance normalized by patch dimension."
        },
    )
    use_raw_frames_in_ttp: bool = field(
        default=False,
        metadata={
            "help": "Apply TTP at raw frame level before temporal merging. "
            "When True, TTP compares each raw frame within temporal_patch_size groups separately, "
            "then ORs the keep decisions. This provides finer-grained temporal duplicate detection. "
            "When False (default), TTP compares at the merged temporal step level."
        },
    )
    ttp_comparison_mode: str = field(
        default="reference",
        metadata={
            "help": "Comparison mode for TTP duplicate detection. "
            ...
            "detect 'runs' of consecutive duplicates, and use ttp_min_run_length to decide removal. "
            "Default is 'reference' for backward compatibility."
        },
    )
    debug_token_removal: bool = field(
        default=False,
        metadata={
            "help": "Enable debug logging for STP/TTP token removal. "
            "When enabled, detailed logs are written to /tmp/ttp_forward_debug_rank{rank}.log "
            "for each GPU rank. Useful for debugging distributed training hangs. "
            "Usage: Set debug_token_removal: true in your training config YAML file."
        },
    )
